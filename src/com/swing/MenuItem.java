...
public class MenuItem extends javax.swing.JPanel {
    ...
    public MenuItem(ModelMenu menu, EventMenu event, EventMenuSelected eventSelected, int index ) {
        ...
        MenuButton fistItem = new MenuButton(menu.getIcon(), "     " + menu.getMenuName());
        fistItem.addActionListener(new ActionListener() {
            @Override
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < getMenu().getSubMenu().length; i++) {
            int y = ((i + 1) * 35 + 40) - 17;
            g2.drawLine(30, y, 38, y);
        }
        
        if( getMenu().getSubMenu().length > 0){
    ...
    private void createArrowButton(Graphics g2) {
        int size = 4;
        int y = 19;
        int x = 170;
